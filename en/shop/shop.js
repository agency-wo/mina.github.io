// [UI-016] shop.js — the EN shop index controller: filter state and the grid.
...
(function(){
  ...
  var STYLE_ALL_LABEL = 'All styles';
  var STYLE_LABELS = {'chronograph':'Chronograph','dress':'Dress','sport':'Sport','digital':'Digital','gold-tone':'Gold-tone','moonphase':'Moonphase'};
  ...
  function watchCard(w){
    var imgHtml = w.image
      ? '<a href="/en/shop/' + w.id + '.html" aria-label="' + w.brand + ' ' + w.model + (SWISS_BRANDS.indexOf(w.brand) !== -1 ? ' Swiss Watch' : '') + '"><picture><source srcset="' + w.image.replace(/\.jpe?g$/i, '.webp') + '" type="image/webp"><img src="' + w.image + '" alt="' + w.brand + ' ' + w.model + (SWISS_BRANDS.indexOf(w.brand) !== -1 ? ' Swiss Watch' : '') + '" loading="lazy"></picture></a>'
  ...
  // [UI-016.i] renderSoon - let the tap paint first, then repaint the grid
